...
def process_audio_segment(audio_file):
    """Process a single audio segment with AAC compression"""
    temp_file = f"{audio_file}_temp.m4a"
    ffmpeg_cmd = [
        'ffmpeg', '-y',
        '-i', audio_file,
        '-c:a', 'aac',     # 使用 AAC 编码器
    ...
    # 注意：需要修改 pydub 以支持 m4a
    audio_segment = AudioSegment.from_file(temp_file, format="m4a")
    os.remove(temp_file)
